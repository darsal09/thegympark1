...
class addStudent
{
	public $mClassInfo  = array( 'startDate' => '', 'endDate' => '', 'price' => '', 'classID' => '', 'classPart' => '' );
	public $mP 			= array( ':first' => '', ':last' => '', ':sex' => '', ':dob' => '', ':hPhone' => '', ':cPhone' => '', ':email' => '' );
	...
	public $mErrors 	= array( ':first' => false, ':last' => false, ':sex' => false, ':dob' => false,
	...
	private $_mRequired 	= array( ':first' => true, ':last' => true, ':sex' => true, ':dob' => true,
							':hPhone' => false, ':cPhone' => false, ':email' => false, 
	...
	public function init()
	{
		...
		$first = helper::getClassParameters( 'First', $this->mClass );
		$last = helper::getClassParameters( 'Last', $this->mClass );
		$this->mClass[ 'endFirstHalf' ] = date( 'm/d/Y', strtotime( $first[ 'end' ] ) );
		$this->mClass[ 'startLastHalf'] = date( 'm/d/Y', strtotime( $last[ 'begin'  ] ) );
		$this->mClass[ 'start_date' ] 	= date( 'm/d/Y', strtotime( $this->mClass[ 'start_date' ] ) );
		$this->mClass[ 'end_date' ] 	= date( 'm/d/Y', strtotime( $this->mClass[ 'end_date' ] ) );
		$this->setLinks();
	}
	...
	private function setLinks()
	{
		...
		$this->mLinks[ sizeof( $this->mLinks ) ] = array( 'title' => 'Program', 'link' => Link::Build( 'admin/programs/'.$this->mProgramID ) );
		$this->mLinks[ sizeof( $this->mLinks ) ] = array( 'title' => 'Category', 'link' => Link::Build( 'admin/programs/'.$this->mProgramID.'/categories/'.$this->mCategoryID ) );
		$this->mLinks[ sizeof( $this->mLinks ) ] = array( 'title' => 'Class Group', 'link' => Link::Build( 'admin/programs/'.$this->mProgramID.'/categories/'.$this->mCategoryID.'/class-groups/'.$this->mClassGroupID ) );
		$this->mLinks[ sizeof( $this->mLinks ) ] = array( 'title' => 'Class', 'link' => Link::Build( 'admin/programs/'.$this->mProgramID.'/categories/'.$this->mCategoryID.'/class-groups/'.$this->mClassGroupID.'/classes/'.$this->mClassID ) );
	}
	...
	private function addStudent()
	{
		...
		$this->mClassInfo = sanitize::filterInputs( 'POST', array( 'startDate' => FILTER_SANITIZE_STRING,
		...
		$this->mClassInfo[ 'classID' ] = $this->mClassID;
		
		$classInfoErrors = sanitize::hasErrors( $this->mClassInfo, array( 'startDate' => true, 'endDate' => true, 'price' => true, 'classPart' => true ), array() );
		
		if( in_array( true, $classInfoErrors ) )
		{
			print_r( $classInfoErrors );
			return;
		}
																		
		$this->mP[ ':first' ] 	= sanitize::sani( $_POST[ 'tgp_student_first' ], 'STRING' );
		$this->mP[ ':last' ] 	= sanitize::sani( $_POST[ 'tgp_student_last' ], 'STRING' );
		...
		{		
			if( administrator::participantExist( $this->mP[ ':first' ], $this->mP[ ':last' ], $this->mP[ ':dob' ], $this->mP[ ':sex' ] ) )
			{
		...
		{
			case 'A':
				$sID = administrator::addParticipant( $this->mClassInfo, $this->mP, $this->mAddress, $this->mSchool, $this->mParent1, $this->mParent2, $this->mCaregiver, $this->mEContact );
				
				if( $sID > 0 )
				{
					if( $this->mClassID > 0 )
						helper::redirect( 'admin/programs/'.$this->mProgramID.'/categories/'.$this->mCategoryID.'/class-groups/'.$this->mClassGroupID.'/classes/'.$this->mClassID );
						
					helper::redirect( 'admin/students/'.$sID );
				}
				else
