...
class classesInfo
{
	...
	{			
		if( isset( $_GET[ 'day' ] ) )
			$this->mDay = sanitize::sani( $_GET[ 'day' ], 'STRING' );
	...
	private function setClasses()
	{
		...
		for( $i = 0; $i < $size; $i++ )
		{
			...
			$C[ $i ][ 'link' ] = Link::Build( 'admin/programs/'.$C[  $i ][ 'tgp_program_id' ].'/categories/'.$C[ $i ][  'category_id' ].'/class-groups/'.$C[ $i ][ 'title_id' ].'/classes/'.$C[ $i ][ 'class_id' ] );
		}
	}
	...
	private function setMenu()
	{
		...
		for( $i = 0; $i < $size; $i++ )
			$this->mObjects[ sizeof( $this->mObjects ) ] = array( 'title' => $days[ $i ][ 'weekday' ], 'link' => Link::Build( 'admin/classes/day/'.$days[ $i ][ 'weekday' ] ) );		
	}
	...
	private function setLinks()
	{
		...
		$this->mLinkToAddClasses =  Link::Build( 'admin/classes/addClasses' );
		$this->mLinkToAllWeek = Link::Build( 'admin/classes' );
	}
}
?>