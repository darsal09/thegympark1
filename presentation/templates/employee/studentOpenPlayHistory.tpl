		...
		<div class="panel-body">
			<div class=panel-group id=accordion>
				...
				{section name=i loop=$obj->mStudentPackages}
						<div id="packages-{$obj->mStudentPackages[ i ].id}" class="panel panel-default">
							<div class=panel-heading>
								<h3 class=panel-title>
										<div class=row>
												<div class=col-md-1 id="packages-{$obj->mStudentPackages[ i ].id}_amount">{$obj->mStudentPackages[ i ].amount}</div>
												<div class=col-md-3>
													<a data-toggle="collapse" data-parent="#accordion" href="#package_{$obj->mStudentPackages[ i ].id}">
														{$obj->mStudentPackages[ i ].title}
													</a>
												</div>
												...
												<div class=col-md-2>
													{helper::getDateWithoutDay( $obj->mStudentPackages[ i ].start )}
												</div>
												<div class=col-md-1>
													{helper::getDateWithoutDay( $obj->mStudentPackages[ i ].end )}
												</div>
												<div id="packages-{$obj->mStudentPackages[ i ].id}_used" class="col-md-1">
													{$obj->mStudentPackages[ i ].used}
												</div>
												<div class=col-md-2 id="packages-{$obj->mStudentPackages[ i ].id}_options">
													{if $obj->mStudentPackages[ i ].status != 'Register'}	
														<a id=removeButton_{$obj->mStudentPackages[ i ].id} class="btn btn-danger" href="javascript:removePackage( {$obj->mStudentPackages[ i ].id} );">remove&raquo;</a>
													{/if}
													<a class="btn btn-success" href="javascript:takeAttendance( {$obj->mStudentPackages[ i ].id } );">Take Attendance&raquo;</a>
												</div>
										</div>								
								</h3>
							</div>
							<div id="package_{$obj->mStudentPackages[ i ].id}" class="panel-collapse collapse out">
								<div class=panel-body>	
									<ul id="package_{$obj->mStudentPackages[ i ].id}_attendance">
										{section name=j loop=$obj->mStudentPackages[ i ].attendance}
											<li id="package_{$obj->mStudentPackages[ i ].id}_attendance_{$obj->mStudentPackages[ i ].attendance[ j ].id}" >{helper::setDateFormat( $obj->mStudentPackages[ i ].attendance[ j ].dates )} <a class="btn btn-danger" href="javascript:removeAttendance( {$obj->mStudentPackages[ i ].id}, {$obj->mStudentPackages[ i ].attendance[ j ].id } );">remove</a></li>
										{/section}
									</ul>
								</div>
							</div>
					...
					<div class="modal-content">
						...
						<div class=modal-content>
							<div class=form-group>
								...
								<input type=text id=start_date value="{date( 'm/d/Y' )}">
							</div>
					...
					<div class="modal-content">
						...
						<div class=modal-body>
								...
								<table class=table style="width:100%;">			
									...
									<input type=hidden id=openPlayID name=openPlayID value="{$obj->mOpenPlay[ 'id']}">
									<input type=hidden id=studentID name=studentID value="{$obj->mStudentID}">
									<thead>
										<tr>
