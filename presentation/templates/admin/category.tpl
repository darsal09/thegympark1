{load_presentation_object filename="category" foldername=admin assign="obj"}
<ol class=breadcrumb>
	...
	<div class="panel panel-default">
		<div class=panel-heading>
			<h1>{$obj->mCategoryInfo.title}
				<small class=pull-right>
					<a href="{$obj->mLinkToDelete}" class="btn btn-default btn-danger btn-lg">Delete&raquo;</a>
					<a href="{$obj->mLinkToEdit}" class="btn btn-default btn-warning btn-lg">Edit&raquo;</a>
					<a href="{$obj->mLinkToSendEmail}" class="btn btn-default btn-success btn-lg">Send Email&raquo;</a>				
				</small>
			</h1>
		</div>
		<div class=panel-body>
		{if $obj->mCategoryEdit}
			<form method=post>
			<input type=hidden name=tgp_category_id value="{$obj->mCategoryInfo.cc_id}">
			<p>
				...
				{section name=i loop=$obj->mPrograms}
					<option 
					{if $obj->mPrograms[ i ].program_id == $obj->mCategoryInfo.program_id}
						selected 
					{/if}
					value="{$obj->mPrograms[ i ].program_id}">{$obj->mPrograms[ i ].title}</option>
				{/section}
			...
			<p>
				...
				<input type=text name=tgp_category_title value="{$obj->mCategoryInfo.title}">
			</p>
			<p>
				<label>Description</label><br/>
				<textarea name=tgp_category_description>{$obj->mCategoryInfo.description}</textarea>
			</p>
		...
		<script>
		<!--
		var colorhex = "{$obj->mCategoryInfo.hexColor}";
		...
		<div id='selectedColor' style="visibility:hidden;position:relative;width:21px;height:21px;background-image:url( '{Link::Build( 'images/selectedcolor.gif' )}');">
		</div>
		<img style='margin-right:2px;' src='{Link::Build( 'images/colormap.gif' )}' usemap='#colormap' alt='colormap' />
		<map id='colormap' name='colormap' onmouseout='mouseOutMap();'>
			...
			<div style='width:200px;padding-top:33px;padding-left:66px;margin-bottom:30px;'>
				<div id='divpreview' style='float:left;height:30px;width:100px;border:1px solid #d4d4d4;border-bottom:none;background-color:{$obj->mCategoryInfo.hexColor}'>
					&nbsp;
				</div>
				<div id='divpreviewtxt' style='float:left;width:50px;padding-left:5px;padding-top:7px;'>{$obj->mCategoryInfo.hexColor}</div>
			</div>
			...
			//-->		
				...
				<p>
					...
					<input type=text id=colorhex name=tgp_category_color value="{$obj->mCategoryInfo.hexColor}">
				</p>
				<p>
		...
		<div class=class_groups>
			<h3>
				...
				<span class=pull-right>
					<a class="btn btn-default btn-primary btn-lg" href="{$obj->mLinkToAddClassTitle}">+Add Class Title&raquo;</a>
				</span>
			</h3>

			{section name=i loop=$obj->mClassTitles}
				<p style="padding:5px;">
					<a href="{$obj->mClassTitles[ i ].link_class_title}">{$obj->mClassTitles[ i ].title} {$obj->mClassTitles[ i ].age_group}</a> 
					<span style="float:right;"><a href="{$obj->mClassTitles[ i ].link_to_add_class}">+Add Class&raquo;</a></span>
				</p>
			{/section}
		</div>
		<p>&nbsp;</p>	
